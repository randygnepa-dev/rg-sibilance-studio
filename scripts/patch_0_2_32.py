...
# Main editor becomes left workspace; fixed right annotations sidebar mirrors approved visual.
...
# Four bottom sections under the main workspace, like the approved visual.
...
# Move real type trim and crossfade controls into Advanced panel.
s=s.replace('let trimLabel = label("TYPE TRIM", size: 10); trimLabel.frame = NSRect(x: 16, y: 22, width: 78, height: 18); p2.addSubview(trimLabel)','let trimLabel = label("TYPE TRIM", size: 10); trimLabel.frame = NSRect(x: 16, y: panelH - 91, width: 78, height: 18); pAdv.addSubview(trimLabel)',1)
s=s.replace('typeTrimSlider = NSSlider(value: 0, minValue: -12, maxValue: 0, target: self, action: #selector(typeTrimChanged)); typeTrimSlider.frame = NSRect(x: 91, y: 19, width: 130, height: 22); typeTrimSlider.isEnabled = false; p2.addSubview(typeTrimSlider)','typeTrimSlider = NSSlider(value: 0, minValue: -12, maxValue: 0, target: self, action: #selector(typeTrimChanged)); typeTrimSlider.frame = NSRect(x: 16, y: panelH - 116, width: advancedW - 72, height: 22); typeTrimSlider.isEnabled = false; pAdv.addSubview(typeTrimSlider)',1)
s=s.replace('typeTrimValue = label("0.0 dB", size: 9, weight: .semibold, color: NSColor(hex: 0x9DB4C5)); typeTrimValue.frame = NSRect(x: 224, y: 22, width: 52, height: 18); p2.addSubview(typeTrimValue)','typeTrimValue = label("0.0 dB", size: 9, weight: .semibold, color: NSColor(hex: 0x9DB4C5)); typeTrimValue.frame = NSRect(x: advancedW - 54, y: panelH - 112, width: 44, height: 18); pAdv.addSubview(typeTrimValue)',1)
s=s.replace('fadeInSlider = NSSlider(value: 12, minValue: 0, maxValue: 120, target: self, action: #selector(fadeChanged)); fadeInSlider.isHidden = true; p2.addSubview(fadeInSlider)','fadeInSlider = NSSlider(value: 12, minValue: 0, maxValue: 120, target: self, action: #selector(fadeChanged)); fadeInSlider.frame = NSRect(x: 16, y: 67, width: advancedW - 68, height: 22); pAdv.addSubview(fadeInSlider)',1)
s=s.replace('fadeOutSlider = NSSlider(value: 12, minValue: 0, maxValue: 120, target: self, action: #selector(fadeChanged)); fadeOutSlider.isHidden = true; p2.addSubview(fadeOutSlider)','fadeOutSlider = NSSlider(value: 12, minValue: 0, maxValue: 120, target: self, action: #selector(fadeChanged)); fadeOutSlider.frame = NSRect(x: 16, y: 37, width: advancedW - 68, height: 22); pAdv.addSubview(fadeOutSlider)',1)
s=s.replace('fadeInValue = label("12 ms", size: 9); fadeInValue.isHidden = true; p2.addSubview(fadeInValue)','fadeInValue = label("IN 12", size: 9); fadeInValue.frame = NSRect(x: advancedW - 48, y: 69, width: 42, height: 18); pAdv.addSubview(fadeInValue)',1)
s=s.replace('fadeOutValue = label("12 ms", size: 9); fadeOutValue.isHidden = true; p2.addSubview(fadeOutValue)','fadeOutValue = label("OUT 12", size: 9); fadeOutValue.frame = NSRect(x: advancedW - 48, y: 39, width: 42, height: 18); pAdv.addSubview(fadeOutValue)',1)

# Separate the spectral display into the lower portion of the editor and waveform into upper area.
...
block=block.replace('let y = plotRect.minY + CGFloat(b) * laneH','let y = plotRect.minY + CGFloat(b) * laneH')
block=block.replace('plotRect.minY + CGFloat(b) * laneH + laneH * 0.5 - 5','plotRect.minY + CGFloat(b) * laneH + laneH * 0.5 - 5')
s=s[:start]+block+s[end:]
start=s.index('    private func drawWaveform(_ m: AudioModel) {')
end=s.index('\n    private func visualGain(', start)
block=s[start:end]
...
# Header labels closer to approved visual.
